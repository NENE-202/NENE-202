![GitHub Banner](A_2D_digital_graphic_banner_in_horizontal_format_i.png)
# Hi, I'm Esther Oziowuh Emmanuel 👋  
Aspiring Data Analyst | Excel • SQL • Python  
Learning with IBM Professional Data Analyst Certificate

## 🔍 What I Do  
- Analyze datasets using SQL & Python  
- Build dashboards with Excel  
- Create end-to-end data analysis projects  
- Learn and practice real-world data skills  

## 🛠️ Tools & Skills  
- Excel (Pivot Tables, Dashboarding)  
- SQL (PostgreSQL)  
- Python (Pandas, Matplotlib, Seaborn)  
- Jupyter Notebook  
- Data Visualization  
- Problem Solving  

## 📂 Featured Projects  
- **Superstore Sales Analysis (Python + SQL)**  
- **Accident Dashboard (Excel)**  
- **SQL Analytics Project – Coming Soon**

## 🌱 Currently Learning  
IBM Professional Data Analyst Certificate

## 📬 Connect with Me  
LinkedIn: https://www.linkedin.com/in/OZIOWUHEMMANUEL/

